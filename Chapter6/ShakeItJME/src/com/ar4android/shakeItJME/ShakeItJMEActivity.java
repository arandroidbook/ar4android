 ...
 * 
 * The code for this example is partially based on 
...
public class ShakeItJMEActivity extends AndroidHarness  {

	private static final String TAG = "ShakeItJMEActivity";
	private Camera mCamera;
	private CameraPreview mPreview;
	private int mDesiredCameraPreviewWidth = 640;
	// Buffer to store camera preview data. Static so that it doesn't need to be
	// re-allocated each frame.
	private byte[] mPreviewBufferYCBCR = null;
	...
	private long mTimeOfLastForce;
	private long mLastTime;
	private float mLastAccelValX = 0;
	private float mLastAccelValY = 0;
	private float mLastAccelValZ = 0f;

	private static final float ACCEL_FORCE_THRESHOLD = 0.03f;//300; // in m/s²
	private static final int TIME_BETWEEN_ACCEL_EVENTS_THRESHOLD = 100; // in ms
	private static final int SHAKE_TIMEOUT = 1000; // in ms
	private static final int SHAKE_DURATION_THRESHOLD = 1000; // in ms
	private static final int ACCEL_EVENT_COUNT_THRESHOLD = 5;
    ...
    class calculateFusedOrientationTask extends TimerTask {
        public void run() {
			...
			if ((com.ar4android.shakeItJME.ShakeItJME) app != null) {
				((com.ar4android.shakeItJME.ShakeItJME) app).setRotationFused((float)(fusedOrientation[2]), (float)(-fusedOrientation[0]), (float)(fusedOrientation[1]));
			}
        }
    }
...
//			Log.d(TAG, "onSensorChanged: " + event.toString());
    ...
    public void detectShake(SensorEvent event) {
    	...
    	if ((now - mTimeOfLastForce) > SHAKE_TIMEOUT) {
    		mShakeCount = 0;
    	} 
    	if ((now - mLastTime) > TIME_BETWEEN_ACCEL_EVENTS_THRESHOLD) {
    		long timeDiff = now - mLastTime;
    		// float speed = Math.abs(event.values[0] + event.values[1] + event.values[2] - mLastAccelValX - mLastAccelValY - mLastAccelValZ) / timeDiff;
    		float curAccForce = Math.abs(event.values[2] - mLastAccelValZ) / timeDiff;
    		if (curAccForce > ACCEL_FORCE_THRESHOLD) {
    			...
    			if ((mShakeCount >= ACCEL_EVENT_COUNT_THRESHOLD) && (now - mTimeOfLastShake > SHAKE_DURATION_THRESHOLD)) {
    				...
    				mShakeCount = 0;
    				Log.d(TAG, "SHAKE with curAccForce " + curAccForce);
    				if ((com.ar4android.shakeItJME.ShakeItJME) app != null) {
    					((com.ar4android.shakeItJME.ShakeItJME) app).onShake();
    				}
    			}
    			mTimeOfLastForce = now;
    		}
    		mLastTime = now;
    		mLastAccelValX = event.values[0];
    		mLastAccelValY = event.values[1];
    		mLastAccelValZ = event.values[2];
    	}    	
    }
...
//		 linearAccelSensor = initSingleSensor(Sensor.TYPE_LINEAR_ACCELERATION, "TYPE_LINEAR_ACCELERATION");
	...
	private final Camera.PreviewCallback mCameraCallback = new Camera.PreviewCallback() {
		public void onPreviewFrame(byte[] data, Camera c) {	
			if (c != null && stopPreview == false) {
				...
				if ((com.ar4android.shakeItJME.ShakeItJME) app != null) {
					((com.ar4android.shakeItJME.ShakeItJME) app)
	...
	public ShakeItJMEActivity() {
		// Set the application class to run
		// appClass = "mygame.Main";
		appClass = "com.ar4android.shakeItJME.ShakeItJME";
